...
import numpy as np
import pandas as pd
from model_PHLF import constants
...
class TransTabFeatureExtractor:
    ...
    def __init__(self,
        ...
        **kwargs,
        ...
        if os.path.exists('./transtab/tokenizer'):
            ...
            # self.tokenizer = BertTokenizerFast.from_pretrained("/home/hci/QYang/YQ_LiverFailure/transtab-main/transtab/Bio_tokenizer")
            self.tokenizer = BertTokenizerFast.from_pretrained("./transtab/Bio_tokenizer_2")
            # self.tokenizer = BertTokenizer.from_pretrained("monologg/biobert_v1.1_pubmed")
            # self.tokenizer = GPT2TokenizerFast.from_pretrained('./transtab/tokenizer')
        else:
    ...
    def __call__(self, x, shuffle=False) -> Dict:
        ...
        if len(num_cols) > 0:
            x_num = x[num_cols]
            x_num = x_num.astype(float).fillna(0) # fill Nan with zero
        ...
        if len(cat_cols) > 0:
            ...
            for col in x_cat.columns:
                cat_cols.append(x_cat[col].unique().astype(str))
            cat_cols = [item for arr in cat_cols for item in arr]

            x_cat_ts = self.tokenizer(cat_cols, padding=True, truncation=True, add_special_tokens=False, return_tensors='pt')

            # x_cat_ts = self.tokenizer(x_cat_str, padding=True, truncation=True, add_special_tokens=False, return_tensors='pt')

            encoded_inputs['x_cat'] = x_cat
            encoded_inputs['cat_cols'] = cat_cols
        ...
        if len(bin_cols) > 0:
            ...
            # x_bin_str = x_bin_str.agg(' '.join, axis=1).str.strip()
            # x_bin_str = x_bin_str.apply(lambda text: text if text else '[NO_ACTIVE_FEATURE]')
            # x_bin_str = x_bin_str.tolist()

            x_bin_ts = self.tokenizer(bin_cols, padding=True, truncation=True, add_special_tokens=False,
                                      return_tensors='pt')
            # x_bin_ts = self.tokenizer(np.array(x_bin_str), padding=True, truncation=True, add_special_tokens=False, return_tensors='pt')
            if x_bin_ts['input_ids'].shape[1] > 0:
                encoded_inputs['x_bin'] = x_bin
                encoded_inputs['bin_cols'] = bin_cols# not all false
    ...
    def forward(self,
        ...
        **kwargs,
        ...
        if x_num is not None and num_col_input_ids is not None:
            num_col_input_ids = num_col_input_ids.to(torch.float64)
        ...
        if x_cat_input_ids is not None:
            ...
            cat_col_emb = self._avg_embedding_by_mask(cat_feat_embedding, cat_att_mask)
            cat_feat_embedding = torch.zeros([x_cat.shape[0], x_cat.shape[1], cat_col_emb.shape[-1]]).to(self.device)
            for idx,idx_row in enumerate(x_cat.index):
                for col_idx, col in enumerate(x_cat.columns):
                    emb_idx = cat_cols.index(x_cat.loc[idx_row,col])
                    cat_feat_embedding[idx,col_idx,:] = cat_col_emb[emb_idx,:]
        ...
        if x_bin_input_ids is not None:
            ...
            bin_embedding = self.word_embedding(x_bin_input_ids.to(self.device))
            bin_col_emb = self._avg_embedding_by_mask(bin_embedding, bin_att_mask)

            bin_feat_embedding = torch.zeros([x_bin.shape[0], x_bin.shape[1], bin_col_emb.shape[-1]]).to(self.device)
            columns_with_one = []
            cout = 0
            for index, row in x_bin.iterrows():
                one_columns = row[row == 1].index.tolist()
                this_row = []
                for col in one_columns:
                    feat_position = bin_cols.index(col)
                    this_row.append(feat_position)
                    bin_feat_embedding[cout, feat_position,:] = bin_col_emb[feat_position,:]
                columns_with_one.append(this_row)
                cout += 1
            # for idx,idx_row in enumerate(x_bin.index):
            #     for col_idx, col in enumerate(x_cat.columns):
            #         emb_idx = cat_cols.index(x_cat.loc[idx_row,col])
        ...
        if cat_feat_embedding is not None:
            ...
            att_mask_list += [torch.ones(cat_feat_embedding.shape[0], cat_feat_embedding.shape[1])]
        if bin_feat_embedding is not None:
            emb_list += [bin_feat_embedding]
            att_mask_list += [torch.tensor(x_bin.values.astype(float))]
...
class TransTabTransformerLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead, attention_weight, dim_feedforward=2048, dropout=0.1, activation=F.relu,
    ...
    def _sa_block(self, x: Tensor,
        ...
        if self.attention_weight:
            ...
            #                                       attn_mask=attn_mask,
            #                                       key_padding_mask=key_padding_mask)
            # torch.save(attn_weights,'/home/hci/QYang/YQ_LiverFailure/transtab-main/attention_map/240522.pth')
            # key_padding_mask_num = 1 - key_padding_mask.to(float)
            # torch.save(key_padding_mask_num, '/home/hci/QYang/YQ_LiverFailure/transtab-main/attention_map/mask.pth')
        else:
            x = self.self_attn(x, x, x,
                               attn_mask=attn_mask,
                               key_padding_mask=key_padding_mask,
                               )[0]
        # key_padding_np = key_padding_mask.detach().cpu().numpy()
        # key_padding_np_sum = key_padding_np.sum(axis=0)
        # key_padding_np_sum = int(key_padding_np.shape[0]) - key_padding_np_sum
        # attn_weights_sum = attn_weights.sum(dim=0)
        # average_attn_weights = attn_weights_sum.detach().cpu().numpy()/key_padding_np_sum
        # average_attn_weights = np.nan_to_num(average_attn_weights)
        # seq_len = average_attn_weights.shape[1]
        # plt.figure(figsize=(10, 10))  # 设置图形大小
        # cax = plt.matshow(average_attn_weights, cmap=plt.cm.Reds)  # 使用'viridis'颜色映射，取第一个样本的注意力矩阵
        # plt.colorbar(cax)
        # plt.savefig('/home/hci/YQ/YQ_LiverFailure/transtab-main/attention_map/map.jpg')
        # plt.show()
...
class TransTabLinearClassifier_gated(nn.Module):
    def __init__(self,
        num_class,
        parent_dim,
        hidden_dim=128) -> None:
        ...
        self.activation = nn.Sigmoid()
        self.norm = nn.LayerNorm(hidden_dim)
        self.gate_fc = nn.Linear(parent_dim-1, hidden_dim)
        self.middle_fc = nn.Linear(hidden_dim, hidden_dim)

    def forward(self, x, parent_prob) -> Tensor:
        x = x[:,0,:] # take the cls token embedding
        x = self.norm(x)

        # x = self.middle_fc(x)
        # x = self.activation(x)
        gate = torch.sigmoid(self.gate_fc(parent_prob.reshape(-1,1)))
        gated_x = x * gate

        logits = self.fc(gated_x)
        # logits = self.activation(logits)
        return logits

class TransTabLinearClassifier_multi_task(nn.Module):
    def __init__(self,
        num_class,
        hidden_dim=128) -> None:
        super().__init__()
...
class TransTabClassifier(TransTabModel):
    ...
    def forward(self, x, y=None):
        ...
        if y is not None:
            ...
            imb_weight = torch.tensor(self.imb_weight)
            self.loss_fn = nn.BCEWithLogitsLoss(pos_weight=imb_weight)
            # ============================================================
            if self.num_class == 2:
                y_ts = torch.tensor(y.values).to(self.device).float()
                loss = self.loss_fn(logits.flatten(), y_ts)

                # loss = self.loss_fn(logits, y_ts.long())
            else:
...
class TransTabClassifier_multi_task(TransTabModel):
    '''The classifier Model subclass from :class:`transtab.modeling_transtab.TransTabModel`.
    ...
    def __init__(self,
        ...
        **kwargs,
        ...
        self.imb_weight = imb_weight
        self.clf_parent = TransTabLinearClassifier_multi_task(num_class=num_class, hidden_dim=hidden_dim)
        self.clf_child = TransTabLinearClassifier(num_class=num_child_class, hidden_dim=hidden_dim)
        if self.num_class > 2:
            self.loss_fn = nn.CrossEntropyLoss(reduction='none')
        else:
            # weights = torch.tensor([1, 2])
            self.loss_fn = nn.BCEWithLogitsLoss(reduction='none')
        if self.num_child_class > 2:
            self.loss_child_fn = nn.CrossEntropyLoss(reduction='none')
        else:
            # weights = torch.tensor([1, 2])
            self.loss_child_fn = nn.BCEWithLogitsLoss(reduction='none')
        self.to(device)
    ...
    def forward(self, x, y):
        ...
        # classifier
        logits = self.clf_parent(encoder_output)
        logits_child = self.clf_child(encoder_output)
        ...
        #     # ============================================================
        #     if self.num_class == 2:
        #         y_ts = torch.tensor(y_parent.values).to(self.device).float()
        #         # loss = self.loss_fn(logits.flatten(), y_ts)
        #
        #         # loss = self.loss_fn(logits, y_ts.long())
        #     else:
        #         y_ts = torch.tensor(y_parent.values).to(self.device).long()
        #         loss = self.loss_fn(logits, y_ts)
        #     # loss = loss.mean()
        # else:
        #     loss = None

        # if y_child is not None:
        #     imb_weight = torch.tensor(self.imb_weight)
        #     if self.num_child_class == 2:
        #         y_ts_child = torch.tensor(y_child.values).to(self.device).float()
        #         loss_child = self.loss_child_fn(logits_child.flatten(), y_ts_child)
        #
        #     else:
        #         y_ts_child = torch.tensor(y_child.values).to(self.device).long()
        #         loss_child = self.loss_child_fn(logits_child, y_ts_child)
        #     loss_child = loss_child.mean()

        parent_logits = logits
        mcm_logits = parent_logits.clone()
        parent_to_child = [[0], [1, 2]]

        for parent_idx in range(logits.shape[1]):
            child_index = parent_to_child[parent_idx]
            if len(child_index) > 0:
                child_logits_subset = logits_child[:,child_index]
                parent_logits_expanded = parent_logits[:,parent_idx].unsqueeze(1)
                combined = torch.cat([parent_logits_expanded, child_logits_subset], dim=1)
                max_values,_ = torch.max(combined, dim=1)
                mcm_logits[:,parent_idx] = max_values

        batch_size = parent_logits.size(0)
        num_parents = parent_logits.size(1)

        parent_prob = torch.sigmoid(mcm_logits)
        child_probs = torch.softmax(logits_child, dim=1)

        total_loss = 0
        epsilon = 1e-8

        y_parent_onehot = F.one_hot(torch.tensor(np.array(y_parent)).long(), self.num_class).float().to(self.device)
        y_child_onehot = F.one_hot(torch.tensor(np.array(y_child)).long(), self.num_child_class).float().to(self.device)

        parent_weight = torch.tensor([1., 1.]).to(self.device)
        parent_pos_wight = torch.tensor([1., 2.]).to(self.device)
        child_weight = torch.tensor([1., 1., 1.]).to(self.device)
        child_pos_wight = torch.tensor([1., 3., 5.]).to(self.device)
        all_loss = 0
        for parent_idx in range(num_parents):
            children = parent_to_child[parent_idx]

            y_A = y_parent_onehot[:,parent_idx]
            mcma = parent_prob[:,parent_idx]

            base_weight = parent_weight[parent_idx]
            pos_wight_factor = parent_pos_wight[parent_idx]

            sample_weights = torch.where(
                y_A == 1,
                base_weight*pos_wight_factor,
                base_weight
            )

            loss_A = F.binary_cross_entropy(mcma, y_A,weight=sample_weights ,reduction='none')

            if len(children) > 0:
                child_lables = y_child_onehot[:,children]
                parent_probs_expended = parent_prob[:,parent_idx].unsqueeze(1)
                child_probs_subset = child_probs[:,children]

                max_probs = torch.where(
                    (y_A==1).unsqueeze(1).expand_as(child_probs_subset),
                    torch.max(parent_probs_expended,child_probs_subset),
                    child_probs_subset
                )

                current_child_weights = child_weight[children]
                current_child_pos_weights = child_pos_wight[children]

                pos_weights = (current_child_weights * pos_wight_factor).unsqueeze(0)
                pos_weights = pos_weights.expand(batch_size, -1)

                neg_weights = current_child_weights.unsqueeze(0).expand(batch_size, -1)

                pos_loss = -child_lables * torch.log(max_probs+epsilon) * pos_weights
                neg_loss = -(1-child_lables)*torch.log(1-parent_probs_expended+epsilon) * neg_weights

                loss_B = (pos_loss+neg_loss).mean()
            else:
                loss_B = torch.zeros_like(loss_A)
            total_loss += (loss_A + loss_B).mean()
        all_loss += total_loss

        return mcm_logits, logits_child, all_loss

class TransTabClassifier_multi_task_onlyclss(TransTabModel):
...
class AttentionSavingWrapper(nn.Module):
    def __init__(self, original_model):
        super().__init__()

        self.original_model = original_model
        self.last_transformer_layer = self.original_model.encoder.transformer_encoder[-1]

    def forward (self, input):
        with torch.no_grad():
            outputs = self.original_model.encoder.transformer_encoder[:-1](input)
            last_input = outputs
            attn_output, attention_weights = self.last_transformer_layer.self_attn(last_input, last_input,
                                                                                   last_input, need_weights=True)
