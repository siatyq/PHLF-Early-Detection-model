...
from Model.evaluator import predict, get_eval_metric_fn, EarlyStopping, get_eval_metric_fn_multi_label
from Model.modeling_PHLF import TransTabFeatureExtractor
from Model.trainer_utils import SupervisedTrainCollator, SupervisedTrainCollator_multi_task,TrainDataset, TrainDataset_multi_label
from Model.trainer_utils import get_parameter_names
from Model.trainer_utils import get_scheduler
...
class Trainer:
    def __init__(self,
        ...
        **kwargs,
        ...
        self._build_dataloader(train_set_list, batch_size, collator=self.collate_fn, num_workers=num_workers)
        ...
        if test_set_list is not None:
            ...
            self._build_dataloader(test_set_list, eval_batch_size, collator=self.collate_fn, num_workers=num_workers,
                                   shuffle=False)
            ]
        else:
        ...
        self.args = {
            ...
            'eval_metric_multi_task':get_eval_metric_fn(eval_metric),
            'eval_metric_name': eval_metric,
            'imb_weight':imb_weight
            }
    ...
    def train(self):
        ...
        for epoch in trange(args['num_epoch'], desc='Epoch'):
            ...
            if self.test_set_list is not None:
                ...
                # eval_res = np.mean(eval_res_list[0][5])
                eval_res = eval_res_list[0][5]
...
class TrainerGradeOnlyClass:
    def __init__(self,
        ...
        if test_set_list is not None:
            self.testloader_list = [
                # self._build_dataloader(testset, eval_batch_size, collator=self.collate_fn, num_workers=num_workers, shuffle=False) for testset in test_set_list
                self._build_dataloader(test_set_list, eval_batch_size, collator=self.collate_fn,
                                       num_workers=num_workers,
                                       shuffle=False)
            ]
        else:
...
class Trainer_multi_task:
    def __init__(self,
        ...
        self.collate_fn = collate_fn
        if collate_fn is None:
            self.collate_fn = SupervisedTrainCollator_multi_task(
                categorical_columns=model.categorical_columns,
                numerical_columns=model.numerical_columns,
                binary_columns=model.binary_columns,
                ignore_duplicate_cols=ignore_duplicate_cols,
        ...
        self.args = {
            ...
            'num_training_steps': self.get_num_train_steps(train_set_list, num_epoch, batch_size),
            'eval_metric': get_eval_metric_fn(eval_metric),
            'eval_metric_multi_task': get_eval_metric_fn_multi_label(eval_metric),
            'eval_metric_name': eval_metric,
            'imb_weight': imb_weight
        }
        self.args['steps_per_epoch'] = int(self.args['num_training_steps'] / (num_epoch * len(self.train_set_list)))
        ...
        for epoch in trange(args['num_epoch'], desc='Epoch'):
            ...
            for dataindex in range(len(self.trainloader_list)):
                for data in self.trainloader_list[dataindex]:
                    self.optimizer.zero_grad()
                    y = data[1].T
                    y = self.y_organizartion(y)
                    logits_parent, logits_child, loss = self.model(data[0], y)
    ...
    def evaluate(self):
        ...
        eval_res_list = []
        for dataindex in range(len(self.testloader_list)):
            y_test_parent, y_test_child, pred_list_parent, pred_list_child, loss_list = [], [], [], [], []
            for data in self.testloader_list[dataindex]:
                if data[1] is not None:
                    result_df = pd.DataFrame()
                    result_df['PHLF'] = pd.Series()
                    result_df['PHLFG'] = pd.Series()
                    df_T = data[1]
                    # 历每一行
                    for row_count, index in enumerate(df_T.T.index):
                        phlf_values = []
                        phlfg_values = []
                        phlf_values = df_T.loc['PHLF', index].sum()
                        phlfg_values = df_T.loc['PHLFG', index].sum()
                        result_df.loc[len(result_df)] = {'PHLF': phlf_values, 'PHLFG': phlfg_values}

                    label_parent = result_df['PHLF']
                    label_child = result_df['PHLFG']
                    if isinstance(label_parent, pd.Series):
                        label_parent = label_parent.values
                        label_child = label_child.values
                    y_test_parent.append(label_parent)
                    y_test_child.append(label_child)
                with torch.no_grad():
                    logits_parent, logits_child, loss = self.model(data[0], result_df)
                if loss is not None:
                    loss_list.append(loss.item())
                if logits_parent is not None:
                     # binary classification
                    pred_list_parent.append(logits_parent.sigmoid().detach().cpu().numpy())
                    pred_list_child.append(torch.softmax(logits_child, -1).detach().cpu().numpy())

            if len(pred_list_parent) > 0:
                pred_list_parent_all = np.concatenate(pred_list_parent, 0)
                pred_list_child_all = np.concatenate(pred_list_child, 0)
                if logits_parent.shape[-1] == 1:
                    pred_all_parent = pred_list_parent_all.flatten()
                    pred_all_child = pred_list_child_all.flatten()

            if self.args['eval_metric_name'] == 'val_loss':
                eval_res = np.mean(loss_list)
            else:
                y_test_parent = np.concatenate(y_test_parent, 0)
                y_test_child = np.concatenate(y_test_child, 0)
                eval_res_parent = self.args['eval_metric'](y_test_parent, pred_list_parent_all[:,1])
                eval_res_child = self.args['eval_metric_multi_task'](y_test_child, pred_list_child_all)

            eval_res_list.append(eval_res_parent)
            eval_res_list.append(eval_res_child)

        return eval_res_list

    def y_organizartion (self,df):
        duplicated_columns = [col for col in df.columns if 'PHLF' in col or 'PHLFG' in col]
        result_df = pd.DataFrame()
        ...
        df_T = df.T
        # 历每一行
        for row_count, index in enumerate(df.index):
            phlf_values = []
            phlfg_values = []
    ...
    def _build_dataloader(self, trainset, batch_size, collator, num_workers=8, shuffle=True):
        trainloader = DataLoader(
            TrainDataset_multi_label(trainset),
            collate_fn=collator,
            batch_size=batch_size,
            shuffle=shuffle,
            num_workers=num_workers,
