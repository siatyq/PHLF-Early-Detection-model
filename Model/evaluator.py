...
from sklearn.metrics import accuracy_score, recall_score, f1_score, roc_auc_score, confusion_matrix, \
    cohen_kappa_score, matthews_corrcoef, precision_score, mean_squared_error
...
def predict(clf, 
    ...
    for i in range(0, len(x_test), eval_batch_size):
        ...
        bs_y_test = y_test[i:i+eval_batch_size]
        with torch.no_grad():
...
def predict_all_prob(clf,
    ...
    clf.eval()
    pred_list, loss_list, encoder_output_list = [], [], []
    for i in range(0, len(x_test), eval_batch_size):
        bs_x_test = x_test.iloc[i:i + eval_batch_size, :]
        bs_y_test = y_test[i:i + eval_batch_size]
        with torch.no_grad():
            logits, loss, encoder_output = clf(bs_x_test, bs_y_test)
        encoder_output_list.append(encoder_output.detach().cpu().numpy())
        if loss is not None:
            loss_list.append(loss.item())
        if logits.shape[-1] == 1:  # binary classification
            pred_list.append(logits.sigmoid().detach().cpu().numpy())
            ...
            pred_list.append(torch.softmax(logits, -1).detach().cpu().numpy())
    pred_all = np.concatenate(pred_list, 0)
    encoder_output = np.concatenate(encoder_output_list, 0)
    if logits.shape[-1] == 1:
        pred_all = pred_all.flatten()
    ...
    if return_loss:
        ...
        return pred_all, encoder_output
...
def predict_fun(clf,
    ...
    x_test = pandas.DataFrame(x_test, columns=col_name)
    with torch.no_grad():
        logits, _ = clf(x_test)
    ...
    if logits.shape[-1] == 1:
        pred_all = pred_all.flatten()
    return pred_all.astype(np.float32)


def predict_multi_task(clf,
            x_test,
            y_test=None,
            return_loss=False,
            eval_batch_size=256,
    ...
    clf.eval()
    pred_list, loss_list = [], []
    pred_list_parent,pred_list_child, loss_list = [], [], []
    for i in range(0, len(x_test), eval_batch_size):
        bs_x_test = x_test.iloc[i:i + eval_batch_size, :]
        bs_y_test = y_test.iloc[i:i + eval_batch_size, :]
        with torch.no_grad():
            mcm_logits, logits_child, total_loss = clf(bs_x_test, bs_y_test)

        if total_loss is not None:
            loss_list.append(total_loss.item())
        # binary classification
        pred_list_parent.append(mcm_logits.sigmoid().detach().cpu().numpy())
        # multi-class classification
        pred_list_child.append(torch.softmax(logits_child, -1).detach().cpu().numpy())

    pred_all_parent = np.concatenate(pred_list_parent, 0)
    pred_all_child = np.concatenate(pred_list_child, 0)
    ...
    if return_loss:
        ...
        return pred_all_parent, pred_all_child
...
def get_eval_metric_fn_multi_label(eval_metric):
    fn_dict = {
        'acc': acc_fn,
        'auc': auc_fn_multi_label,
        'mse': mse_fn,
        'val_loss': None,
    }
    return fn_dict[eval_metric]
...
def auc_fn(y, p):
    ...
    y_p = np.argmax(p, -1)
    y_preds2 = 1 - p
    y_preds = np.concatenate([y_preds2.reshape(-1, 1), p.reshape(-1, 1), ], axis=1)
...
class EarlyStopping:
    ...
    def __init__(self, patience=7, verbose=False, delta=0, output_dir='ckpt', trace_func=None, less_is_better=False):
        """
        Args:
