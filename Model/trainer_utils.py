...
from .modeling_PHLF import TransTabFeatureExtractor
...
class TrainDataset_multi_label(Dataset):
    def __init__(self, trainset):
        self.x, self.y = trainset
    ...
    def __getitem__(self, index):
        ...
        if self.y is not None:
            # y = self.y.iloc[index - 1:index]
            y_parent = self.y[0].iloc[index - 1:index]
            y_child = self.y[1].iloc[index - 1:index]
        else:
            y = None
            y_parent = None
            y_child = None
...
class SupervisedTrainCollator_multi_task(TrainCollator):
    def __init__(self,
                 categorical_columns=None,
                 numerical_columns=None,
                 binary_columns=None,
    ...
    def __call__(self, data):
        x = pd.concat([row[0] for row in data])
        y = pd.concat([pd.DataFrame(row[1]) for row in data])
        inputs = self.feature_extractor(x)
        return inputs, y
